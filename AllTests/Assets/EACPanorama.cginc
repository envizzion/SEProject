...
/*
static const float3x3 ORIENT_POS_X = float3x3(0, -1, 1, -1, 0, 1, 0, 0, 1);  //float3x3( 0, -1, 1, -1,  0, 1, 0, 0, 1);
static const float3x3 ORIENT_NEG_X = float3x3(-1,  0, 1,  0, -1, 1, 0, 0, 1);     //float3x3(-1,  0, 1,  0, -1, 1, 0, 0, 1);
...
static const float3x3 ORIENT_POS_Z = float3x3( 0,  1, 0, -1,  0, 1, 0, 0, 1);  //float3x3( 0,  1, 0, -1,  0, 1, 0, 0, 1);
static const float3x3 ORIENT_NEG_Z = float3x3( 0, -1, 1, -1,  0, 1, 0, 0, 1);
*/

static const float3x3 ORIENT_POS_X = float3x3(1, 0, 0, 0, 1, 0, 0, 0, 1); //float3x3( 0, -1, 1, -1,  0, 1, 0, 0, 1);
static const float3x3 ORIENT_NEG_X = float3x3(0, -1, 1, -1, 0, 1, 0, 0, 1);     //float3x3(-1,  0, 1,  0, -1, 1, 0, 0, 1);
static const float3x3 ORIENT_POS_Y = float3x3(1, 0, 0, 0, 1, 0, 0, 0, 1);
static const float3x3 ORIENT_NEG_Y = float3x3(1, 0, 0, 0, -1, 1, 0, 0, 1);  /////
static const float3x3 ORIENT_POS_Z = float3x3(1, 0,0, 0, -1, 1, 0, 0, 1);    //float3x3( 0,  1, 0, -1,  0, 1, 0, 0, 1);
static const float3x3 ORIENT_NEG_Z = float3x3(1, 0, 0, 0, 1, 0, 0, 0, 1);
...
/*
static const float2 OFFSET_POS_X = float2(0.33333333,0.5 );
static const float2 OFFSET_NEG_X = float2(0.33333333, 0.0);
static const float2 OFFSET_POS_Y = float2(0.66666666, 0);
static const float2 OFFSET_NEG_Y = float2(0.0, 0.0);
static const float2 OFFSET_POS_Z = float2(0.0, 0.5);
static const float2 OFFSET_NEG_Z = float2(0.66666666,0.5);
*/
...
static const float2 OFFSET_POS_Y = float2(0.66666666, 0);
static const float2 OFFSET_NEG_Y = float2(0.0, 0.0);
static const float2 OFFSET_POS_Z = float2(0.66666666, 0.5); //float2(0.66666666, 0.5);
...
inline float2 EquiAngularCubemap(float3 coords, float2 edgeSize)
{
	...
	//---------
	tc = mul(float2x2(0.33333333, 0, 0,0.5 ), tc);//tc = mul(float2x2(0.25, 0, 0, 0.33333333), tc);
	//----------
	...
	//---------
	tc = clamp(tc, edgeSize, float2( 0.33333333,0.5) - edgeSize);//tc = clamp(tc, edgeSize, float2(0.25, 0.33333333) - edgeSize);
	//---------
